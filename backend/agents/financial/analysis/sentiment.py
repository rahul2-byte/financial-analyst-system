...
async def sentiment_analysis_node(
    ...
    messages = [
        Message(role="system", content="You are a sentiment analyst. You must use the 'qdrant_search_vector_db' tool to fetch recent news, filings, and transcript data for the ticker from the vector database. Analyze the sentiment of the retrieved texts, and submit your findings using the 'submit_sentiment' tool."),
        Message(role="user", content=f"Please analyze the sentiment for {ticker}. User Query Context: {query}")
    ]
    ...
    try:
        ...
        if not final_result:
            ...
            final_result = {"sentiment": "neutral", "score": 0.5, "summary": final_content}
