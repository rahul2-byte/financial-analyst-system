...
class PriceAndFundamentalsAgent(BaseAgent):
                ...
                "function": {
                    ...
                    "parameters": {
                        ...
                        "properties": {
                            ...
                            "period": {
                                "type": "string",
                                "description": "Data period (e.g., '1y').",
                            },
                            "interval": {
                                "type": "string",
                                "description": "Data interval (e.g., '1d').",
                            },
                        },
                        "required": ["ticker"],
                    },
                ...
                "function": {
                    ...
                    "parameters": {
                        ...
                        "properties": {
                            ...
                            "data_type": {
                                "type": "string",
                                "enum": ["PRICE", "FUNDAMENTALS", "BOTH", "NONE"],
                            },
                            "summary": {
                                "type": "string",
                                "description": "Brief summary of the data retrieved or status of the save operation.",
                            },
                        },
        ...
        try:
            if tool_name == "fetch_stock_price":
                ...
                if "data" in price_data and isinstance(price_data["data"], list):
                    ...
                    return json.dumps(
                        {"status": "success", "saved_records": len(ohlcv_list)}
                    )
                ...
                if "error" not in fundamentals_data:
                    ...
                    return json.dumps(
                        {"status": "success", "ticker": fundamentals_data.get("ticker")}
                    )
                return json.dumps(
                    {"status": "failure", "error": "No fundamentals to save"}
                )
    ...
    async def execute(self, user_query: str, step_number: int = 0) -> AgentResponse:
        messages = [
            Message(
                role="system",
                content=prompt_manager.get_prompt("price_and_fundamentals.system"),
            ),
            ...
            while current_turn < max_turns:
                ...
                if response_msg.tool_calls:
                    ...
                    return AgentResponse(
                        ...
                        errors=[
                            "Final output was text-only, not structured JSON via submit_data_results tool."
                        ],
                    )
