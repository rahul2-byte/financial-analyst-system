...
def horizon_from_text(value: Any) -> str:
    text = str(value).lower()
    if any(
        token in text for token in ("short-term", "short term", "intraday", "swing")
    ):
        return "short"
    if any(
        token in text
        for token in ("long-term", "long term", "multi-year", "structural")
    ):
        return "long"
...
def intensity_from_text(value: Any) -> int:
    ...
    if any(token in text for token in ("buy", "bullish", "positive")):
        return 1
    if any(
        token in text for token in ("strong sell", "very bearish", "aggressive sell")
    ):
        return -2
    ...
    for i in range(len(claims)):
        for j in range(i + 1, len(claims)):
            ...
            if left_ev == 0 or right_ev == 0:
                ...
                evidence_factor = min(left_ev, right_ev) / max(
                    1, max(left_ev, right_ev)
                )
...
def top_metric_drivers(
    tool_registry: list[dict[str, Any]], limit: int = 3
) -> list[str]:
