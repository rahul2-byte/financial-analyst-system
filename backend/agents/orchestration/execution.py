...
async def execute_level_node(
    ...
    for step, result in zip(next_level, results):
        ...
        if isinstance(result, Exception):
            ...
            all_errors.append(
                f"Step {step.step_number} ({agent_name}) failed: {result}"
            )
            ...
            all_errors.extend(
                [
                    f"Step {step.step_number} ({agent_name}) failed: {err}"
            ...
            if output_val is None and outputs:
                ...
                logger.warning(
                    f"Output for {agent_name} (step {step.step_number}) not found in agent_outputs. Available keys: {list(outputs.keys())}"
                )
