    ...
    async def _connect(self, name: str, params: StdioServerParameters):
        try:
            ...
            for t in mcp_tools.tools:
                ...
                from app.core.tools.tool_system import ToolDefinition, ToolNamespace, tool_registry
                ...
                tool_executor.register_handler(tool_def.full_name, make_handler(name, t.name))
                
            logger.info(f"Connected to MCP server: {name} and loaded {len(mcp_tools.tools)} tools.")
        except Exception as e:
