...
async def test_research_plan_uses_rag_for_sentiment_and_contrarian(monkeypatch):
    ...
    monkeypatch.setattr(
        node_module, "EmbeddingService", MagicMock(return_value=mock_embedding_service)
    )
    ...
    state = {
        ...
        "fetched_data": {"news": [{"title": "RSS Title", "summary": "RSS Summary"}]},
    ...
    assert any(
        "Chunk 1 text" in str(item) for item in contrarian_params["sentiment_data"]
    )
    assert not any(
        "RSS Title" in str(item) for item in contrarian_params["sentiment_data"]
    )
...
async def test_research_plan_uses_placeholder_when_rag_empty(monkeypatch):
    ...
    sentiment_params = tasks["sentiment_analysis"]["parameters"]
    assert (
        "Insufficient qualitative evidence found for research analysis"
        in sentiment_params["text"]
    )
    assert "RSS Title" not in sentiment_params["text"]
