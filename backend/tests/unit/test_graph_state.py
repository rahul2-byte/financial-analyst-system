...
class TestMergeDicts:
    ...
    def test_tool_registry_concatenation(self):
        """Two nodes return tool_registry, should concatenate."""
        left = {"tool_registry": [{"name": "tool1", "func": "func1"}]}
        right = {"tool_registry": [{"name": "tool2", "func": "func2"}]}

        result = merge_dicts(left, right)

        expected = {
            "tool_registry": [
                {"name": "tool1", "func": "func1"},
                {"name": "tool2", "func": "func2"},
            ]
        }
        assert result == expected

    def test_executed_steps_concatenation(self):
        """Two steps executed, should concatenate."""
        left = {"executed_steps": [{"step": 1, "agent": "planner"}]}
        right = {"executed_steps": [{"step": 2, "agent": "executor"}]}
    ...
    def test_agent_outputs_merge_preserves_both_steps(self):
        """Different step outputs should both be preserved."""
        left = {"agent_outputs": {"step_1": {"data": "from_step_1"}}}
        right = {"agent_outputs": {"step_2": {"data": "from_step_2"}}}

        result = merge_dicts(left, right)

        expected = {
            "agent_outputs": {
                "step_1": {"data": "from_step_1"},
                "step_2": {"data": "from_step_2"},
            }
        }
        assert result == expected
