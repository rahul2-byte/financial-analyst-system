...
def ndcg_at_k(ranked_ids: list[str], relevant_ids: set[str], k: int) -> float:
    ...
    ideal = sum(1.0 / log2(rank + 1) for rank in range(1, min(k, len(relevant_ids)) + 1))
...
def _numeric_provenance(task: dict[str, Any], result: dict[str, Any]) -> tuple[int, int]:
...
def evaluate_case(task: dict[str, Any], result: dict[str, Any]) -> dict[str, Any]:
    ...
    major_claims = [claim for claim in claims if isinstance(claim, dict) and claim.get("importance") == "major"]
    ...
    checked_citations = citations & required_citations if required_citations else citations
    ...
    return {
        ...
        "unsupported_major_claim_rate": _rate(len(major_claims) - len(supported_major), len(major_claims)),
        ...
        "loop_or_error": bool(result.get("budget_exhausted", False) or result.get("loop_error", False)),
...
def aggregate_metrics(case_metrics: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "task_completion_rate": _rate(sum(bool(item.get("task_completed")) for item in case_metrics), count),
        "major_claim_support_rate": _mean([float(item["major_claim_support_rate"]) for item in case_metrics]),
        "numeric_provenance_rate": _mean(numeric),
        "citation_precision": _mean([float(item["citation_precision"]) for item in case_metrics]),
        "citation_recall": _mean([float(item["citation_recall"]) for item in case_metrics]),
        "risk_f1": _mean([float(item["risk_f1"]) for item in case_metrics]),
        "valid_plan_rate": _rate(sum(bool(item.get("valid_plan")) for item in case_metrics), count),
        "fail_closed_rate": _rate(sum(bool(item.get("fail_closed")) for item in case_metrics), count),
        "loop_error_rate": _rate(sum(bool(item.get("loop_or_error")) for item in case_metrics), count),
        "evidence_selection": {
            "recall_at_5": _mean([float(item.get("recall_at_5", 0.0)) for item in evidence]),
            "recall_at_10": _mean([float(item.get("recall_at_10", 0.0)) for item in evidence]),
            "mrr_at_10": _mean([float(item.get("mrr_at_10", 0.0)) for item in evidence]),
            "ndcg_at_10": _mean([float(item.get("ndcg_at_10", 0.0)) for item in evidence]),
        },
    }
